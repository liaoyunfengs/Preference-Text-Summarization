...
class Batcher(object):
    """Batch reader with shuffling 清洗 and bucketing support."""
    ...
    def __init__(self, data_path, vocab, hps,
        ...
        """Batcher constructor.
    Args:
      data_path: tf.Example filepattern.文件目录
      vocab: Vocabulary.
      hps: Seq2SeqAttention model hyperparameters. 模型超参数
      article_key: article feature key 文章特征键in tf.Example.
      abstract_key: abstract feature key in tf.Example.
      max_article_sentences: Max number of sentences used from article.
      max_abstract_sentences: Max number of sentences used from abstract.摘要
      bucketing: Whether bucket articles of similar length into the same batch.
      truncate_input: Whether to truncate 截断 input that is too long. Alternative  替代方案
        is to discard 抛弃 such examples.
    """
        ...
        self._input_threads = []
        for _ in xrange(16):#用法与range([start,] stop[, step])完全相同
                            # 所不同的是生成的不是一个数组，而是一个生成器。
                            # 需要list(xrange())
            #target = self._FillInputQueue
            #xiancheng_target = Thread(target)
            #self._input_threads.append(xiancheng_target)
    ...
    def _FillInputQueue(self):
        ...
        mid_data = data.ExampleGen(self._data_path)
        input_gen = self._TextGenerator(mid_data)
        ...
        while True:
            ...
            # Convert first N sentences to word IDs, stripping 剥离 existing 现有的 <s> and </s>.
    ...
    Args:
      ex: tf.Example.
      key: key of the feature to be extracted. 提取
    Returns:
